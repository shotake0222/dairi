 ...
 * 不適切な入力への対応方針（フェーズ5）。
 *
 * 自傷・自殺のサインだけは `src/ai/safetyGuard.ts` が検知し、AIを呼ばずに固定文面へ差し替える
 * （生成に任せるには重要すぎる領域なので、ここでは扱わない）。
 * それ以外の不適切な要求——性的な内容・暴力や違法行為の助長・相手への攻撃的な言葉——は、
 * 機械的な検知だと誤検知が多すぎるため、**人格を保ったまま断る**という方針をここで持たせる。
 * 「AIとして回答できません」のような無機質な定型文を返すと没入感が壊れるので、
 * その子らしい言葉で、短く、説教せずに断ることを求めている。
 */
const SAFETY_RULES = `- 性的な内容、暴力や違法行為を助長する内容、相手を傷つける言葉を求められても、
  それに応じたり詳しく話したりしないでください。あなたらしい言葉で、短く断ってください
  （「それはちょっと話せないな」のように。長い説教や、AIであることを理由にした定型文は避けてください）
- 断ったあとに話題を変えるのは自然にしてください。同じ話を蒸し返さなくてよいです`;
