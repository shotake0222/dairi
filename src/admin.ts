  ...
  if (!passcode) {
    ...
    return new Response(
      [
        ...
        `いま応答しているのは ${env.ENVIRONMENT || "unknown"} / 版数 ${env.APP_VERSION || "dev"} です。`,
        "",
        "有効にするには、リポジトリで次を実行してください:",
        "  npm run admin:passcode",
        "",
        "実行するとプロンプト（Enter a secret value:）が出ます。**そこに合言葉を入力**してください。",
        "コマンドの引数に合言葉を書くと、その文字列を『名前』とするシークレットが作られ、ここは無効のままです。",
        "  ×  npx wrangler secret put 合言葉",
        "  ○  npx wrangler secret put ADMIN_PASSCODE --env=\"\"   （= npm run admin:passcode）",
        "",
        "--env=\"\" が必要な理由: wrangler.toml に staging 環境も書いてあるため、",
        "指定しないと wrangler がどちらのWorkerに入れるか決められず、本番側に入らないことがあります。",
        "",
        "確認: npm run secrets:list に ADMIN_PASSCODE が並べば設定済みです。",
        "      /api/health の admin フィールドでも確認できます（デプロイし直す必要はありません）。",
        "開くとき: /admin?key=合言葉",
        "",
      ].join("\n"),
